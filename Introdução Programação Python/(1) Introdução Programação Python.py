...
# Escolhendo as cores da interface: Tools -> Preferences
...
# Exemplos (digitar diretamente na linha de comando no console sem o #):
# pip install pandas
# pip install numpy
...
#%% Instalando pacotes: exemplos

# Vamos instalar pacotes que estão listados no script
# Neste caso, note que adicionamos uma exclamação antes do pip
# O pip é um instalador que deve ser executado diretamente na linha de comando
# A exclamação tem a função de passar o pip diretamente para a linha de comando
...
# Warnings são avisos, mensagens de alerta que normalmente não travam a execução 
...
ipca = pd.read_csv("https://api.bcb.gov.br/dados/serie/bcdata.sgs.433/dados?formato=csv&dataInicial=01/01/2022&dataFinal=31/03/2024",
                   sep=";", decimal=",")
...
print(calcular_area(10, 10))
print(calcular_area(20, 15), "m²")
print(calcular_area(50, 30), "m²")
...
def coef_var(x):
  ...
  return np.round(coeficiente, decimals=4)

#%% Testando a função

variavel_cv = pd.Series([10, 25, 40, 35, 15, 28, 31])

print(coef_var(variavel_cv), "%")
...
# Para trocar apenas um nome pela posição da coluna:

pisa.columns.array[1] = 'grupos_paises'
...
ax = sns.countplot(data=comercio, x="market", order=["APAC", "LATAM", "EU", "US", "Africa", "EMEA", "Canada"], palette='viridis')
...
ax = sns.countplot(data=comercio, y="market", 
...
sns.barplot(data=comercio_agrupado, x='category', y='profit', palette='rocket')
...
ax = sns.histplot(data = hist_vendas, x = "sales", bins=range(0,1100,100), color = 'blue', alpha = 0.6, kde = True)
ax.bar_label(ax.containers[0], fontsize=8)
plt.xlabel('Valor das Vendas',fontsize=12)
...
plt.title("Receita de Vendas")
plt.xlabel('Ano',fontsize=12)
plt.ylabel('Receita Anual de Vendas',fontsize=12)
...
plt.xlabel('Produto A',fontsize=12)
